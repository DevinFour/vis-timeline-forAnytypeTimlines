...
class RangeItem extends Item {
  ...
  repositionX(limitSize) {
    ...
    switch (align) {
      ...
      default: // 'auto'
        // CUSTOM FIX: For statistics with React content, disable content repositioning
        // This prevents the "hanging left" behavior during panning
        if (
          this.data.className &&
          (this.data.className.includes("statistic-placeholder") || 
           this.data.className.indexOf("statistic-placeholder") !== -1)
        ) {
          // For statistics, keep content always at position 0 (no repositioning)
          contentStartPosition = 0;
          console.log('🔧 Fixed positioning for statistic item:', this.data.id);
        } else {
          // Original vis-timeline behavior for non-statistics
